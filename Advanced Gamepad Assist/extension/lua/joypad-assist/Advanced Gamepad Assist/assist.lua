...
local calibrationSmoother      = lib.SmoothTowards:new(1.3, 1.0, -1.0, 1.0, 0.0) -- Smooths the initial steering input
...
local function performCalibration(inputData, vehicle, inverseBodyTransform, dt)
    ...
    if calibrationStage < 2 then
        ...
        if math.isNaN(calibrationRateMult) then
            calibrationRateMult = math.clamp(ac.getSim().fps / 60.0, 0.5, 1.5)
        end
        ...
        if calibrationDelay < (0.7 / calibrationRateMult) or currentAbsAngle < 0.2 then
            calibrationDelay = calibrationDelay + dt
            inputData.steer = sanitizeInput(calibrationSmoother:getWithRateMult(0.05, dt, 5.0))
            return false
        end
        ...
        if not calibrationSavedWheels then
            ...
            if math.abs(rAxlePos.x) > 0.01 then
                ac.setMessage("Advanced Gamepad Assist", "Error - The average rear wheel position seems off-center.")
            end
        end
        ...
        if calibrationStage == 0 then
            ...
            inputData.steer = sanitizeInput(calibrationSmoother:getWithRateMult(1, dt, calibrationRateMult))
        ...
        elseif calibrationStage == 1 then
            if math.isNaN(vehicleSteeringLock) then
                ...
                if vehicleSteeringLock < 3.0 or vehicleSteeringLock > 90.0 then
            ...
            inputData.steer = sanitizeInput(calibrationSmoother:getWithRateMult(0, dt, 5.0))
...
local function calcCorrectedSteering(vData, targetFrontSlipDeg, initialSteering, absInitialSteering, steeringRateMult, dt)
    ...
    local clampedFWheelVelAngle = lib.clampEased(inputSign * fWheelVelHAngle, -vData.steeringLockDeg - 15.0, angleSubLimit, 3.5 / (vData.steeringLockDeg + 15.0 + angleSubLimit)) -- Limiting how much the front velocity angle can affect the steering limit
    ...
    local selfSteerForce      = math.clamp(selfSteerSmoother:get(lib.clampEased(correctionBase, -selfSteerCap, selfSteerCap, selfSteerCapT), dt) + dampingForce, -2, 2) * selfSteerStrength
    ...
    local slipCorrection      = 1.01 -- // FIXME some cars change with speed, find out why
...
local function updateDisplayValues(vData, assistFadeIn, dt)
    if assistFadeIn < 1e-15 then
...
function script.update(dt)
    ...
    local initialSteering    = steeringSmoother:getWithRateMult(vData.inputData.steerStickX, dt, steeringRateMult) -- Steering input with no processing (except smoothing)
    local absInitialSteering = absSteeringSmoother:getWithRateMult(math.abs(vData.inputData.steerStickX), dt, steeringRateMult)  -- Absolute steering input with no processing (except smoothing)
    ...
    if uiData.assistEnabled then
        ...
        vData.inputData.steer    = sanitizeInput(normalizedSteeringToInput(desiredSteering, vData.steeringCurveExponent)) -- Final steering input sent to the car
    end

    updateDisplayValues(vData, assistFadeIn, dt)
