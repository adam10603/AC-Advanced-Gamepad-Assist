...
local tooltips = {
    ...
    autoShiftingMode         = "Default = AC's default shifting, no change.\n\nManual = enables custom rev-matching and clutch logic, but only allows manual gear shifting.\n\nAutomatic = enables custom rev-matching and clutch logic, and also automatically shifts gears using a custom algorithm. This mode can still accept manual shifting inputs for temporary gear overrides.\n\nIMPORTANT: Options other than 'Default' only work properly if 'Automatic shifting' is DISABLED in AC's own assist settings!",
    ...
    dampingStrength          = "This is an advanced setting, and in most cases I'd just leave it at a similar value to 'Response'.\n\nDamping adds some steering input that counteracts the car's yaw rotation which results in more stability.\nHigher 'Response' and 'Max angle' settings require more damping to stop the car from wobbling, especially at high speed.\nThe damping force is not limited by the 'Max angle' setting.",
